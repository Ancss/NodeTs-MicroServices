...
describe('Email Consumer', () => {
  ...
  describe('consumeAuthEmailMessages method', () => {
    it('should be called', async () => {
      ...
      expect(connectionChannel!.assertExchange).toHaveBeenCalledWith('nodets-email-notification', 'direct');
      expect(connectionChannel!.assertQueue).toHaveBeenCalledTimes(1);
      expect(connectionChannel!.consume).toHaveBeenCalledTimes(1);
      expect(connectionChannel!.bindQueue).toHaveBeenCalledWith('auth-email-queue', 'nodets-email-notification', 'auth-email');
    });
  });
  ...
  describe('consumeOrderEmailMessages method', () => {
    it('should be called', async () => {
      ...
      expect(connectionChannel!.assertExchange).toHaveBeenCalledWith('nodets-order-notification', 'direct');
      expect(connectionChannel!.assertQueue).toHaveBeenCalledTimes(1);
      expect(connectionChannel!.consume).toHaveBeenCalledTimes(1);
      expect(connectionChannel!.bindQueue).toHaveBeenCalledWith('order-email-queue', 'nodets-order-notification', 'order-email');
    });
  });
});
