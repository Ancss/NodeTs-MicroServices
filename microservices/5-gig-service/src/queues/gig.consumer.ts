...
import { winstonLogger } from '@ancss/ancss-shared';
...
const consumeGigDirectMessage = async (channel: Channel): Promise<void> => {
  try {
    ...
    const exchangeName = 'ancss-update-gig';
    const routingKey = 'update-gig';
    const queueName = 'gig-update-queue';
    await channel.assertExchange(exchangeName, 'direct');
    const ancssQueue: Replies.AssertQueue = await channel.assertQueue(queueName, { durable: true, autoDelete: false });
    await channel.bindQueue(ancssQueue.queue, exchangeName, routingKey);
    channel.consume(ancssQueue.queue, async (msg: ConsumeMessage | null) => {
...
const consumeSeedDirectMessages = async (channel: Channel): Promise<void> => {
  try {
    ...
    const exchangeName = 'ancss-seed-gig';
