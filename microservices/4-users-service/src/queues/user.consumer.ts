...
import { IBuyerDocument, ISellerDocument, winstonLogger } from '@ancss/nodets-shared';
...
const consumeBuyerDirectMessage = async (channel: Channel): Promise<void> => {
  try {
    ...
    const exchangeName = 'nodets-buyer-update';
    const routingKey = 'user-buyer';
    const queueName = 'user-buyer-queue';
    await channel.assertExchange(exchangeName, 'direct');
    const nodetsQueue: Replies.AssertQueue = await channel.assertQueue(queueName, { durable: true, autoDelete: false });
...
const consumeSellerDirectMessage = async (channel: Channel): Promise<void> => {
  try {
    ...
    const exchangeName = 'nodets-seller-update';
...
const consumeReviewFanoutMessages = async (channel: Channel): Promise<void> => {
  try {
    ...
    const exchangeName = 'nodets-review';
    const queueName = 'seller-review-queue';
    await channel.assertExchange(exchangeName, 'fanout');
    const nodetsQueue: Replies.AssertQueue = await channel.assertQueue(queueName, { durable: true, autoDelete: false });
    await channel.bindQueue(nodetsQueue.queue, exchangeName, '');
    channel.consume(nodetsQueue.queue, async (msg: ConsumeMessage | null) => {
      const { type } = JSON.parse(msg!.content.toString());
